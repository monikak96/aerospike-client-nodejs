 ...
 * var key = new Key('test', 'demo', 'key1')
 *
 * client.get(key, (error, record, metadata) => {
 *   if (error) {
 *     // handle failure
 *   } else {
 *     // handle success
...
Client.prototype.get = function (key, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('getAsync', [key, policy], (err, record, metadata) => {
      callback(err, record, metadata, key)
    })
  } else {
    return this.sendCommand('getAsync', [key, policy])
      .then((result) => Promise.resolve(result.concat([key])))
  }
}
...
Client.prototype.info = function (request, host, policy, infoCb, doneCb) {
  ...
  if (!this.isConnected(false)) {
    let result = this.sendError(infoCb, 'Not connected.')
...
Client.prototype.put = function (key, record, metadata, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('putAsync', [key, record, metadata, policy], (err) => {
      callback(err, key)
    })
  } else {
    return this.sendCommand('putAsync', [key, record, metadata, policy])
...
Client.prototype.sendCommand = function (cmd, args, callback) {
  ...
  if (callback) {
    ...
    args.push(function (err, arg1, arg2, arg3) {
      err = AerospikeError.fromASError(err, cmdStackTrace)
      if (sync) {
